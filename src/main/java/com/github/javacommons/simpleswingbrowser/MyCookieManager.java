 ...
 * and rejecting cookies. A CookieManager is initialized with a {@link MyCookieStore}
 ...
 *       but implement user's own {@link MyCookieStore} and {@link CookiePolicy}
...
public class MyCookieManager extends CookieHandler
{
    ...
    *****/
    ...
    private MyCookieStore cookieJar = null;
     ...
     */
    /*****
    public MyCookieManager(MyCookieStore store,
    ...
    public MyCookieManager(MyCookieStore store)
    {
        ...
        if (store == null) {
            cookieJar = new MyInMemoryCookieStore();
        } else {
            cookieJar = store;
        }
    }
        ...
        throws IOException
    {
        ...
        List<MyHttpCookie> cookies = new java.util.ArrayList<MyHttpCookie>();
        String path = uri.getPath();
        if (path == null || path.isEmpty()) {
            path = "/";
        }
        for (MyHttpCookie cookie : cookieJar.get(uri)) {
    ...
    PlatformLogger logger = PlatformLogger.getLogger("java.net.CookieManager");
        for (String headerKey : responseHeaders.keySet()) {
            ...
            for (String headerValue : responseHeaders.get(headerKey)) {
                try {
                    List<MyHttpCookie> cookies;
                    try {
                        cookies = MyHttpCookie.parse(headerValue);
                    ...
                    for (MyHttpCookie cookie : cookies) {
    ...
    private boolean shouldAcceptInternal(URI uri, MyHttpCookie cookie) {
        /*****
        try {
     ...
     */
    private List<String> sortByPath(List<MyHttpCookie> cookies) {
    ...
    static class CookiePathComparator implements Comparator<MyHttpCookie> {
        public int compare(MyHttpCookie c1, MyHttpCookie c2) {
            if (c1 == c2) return 0;
            if (c1 == null) return -1;
            if (c2 == null) return 1;
