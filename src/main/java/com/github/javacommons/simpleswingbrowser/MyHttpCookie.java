 ...
 * @since 1.6
 */
public final class MyHttpCookie implements Cloneable {
    // ---------------- Fields --------------
     ...
     * @see #setVersion
     */
    public MyHttpCookie(String name, String value) {
        this(name, value, null /*header*/);
    }

    private MyHttpCookie(String name, String value, String header) {
        name = name.trim();
     ...
     */
    public static List<MyHttpCookie> parse(String header) {
    ...
    // class.
    private static List<MyHttpCookie> parse(String header, boolean retainHeader) {
        ...
        List<MyHttpCookie> cookies = new java.util.ArrayList<>();
        ...
        if (version == 0) {
            ...
            MyHttpCookie cookie = parseInternal(header, retainHeader);
            cookie.setVersion(0);
            cookies.add(cookie);
        } else {
            // rfc2965/2109 cookie
            ...
            for (String cookieStr : cookieStrings) {
                MyHttpCookie cookie = parseInternal(cookieStr, retainHeader);
                cookie.setVersion(1);
                cookies.add(cookie);
            }
        }
    ...
    public boolean equals(Object obj) {
        if (obj == this)
            return true;
        if (!(obj instanceof MyHttpCookie))
            return false;
        MyHttpCookie other = (MyHttpCookie)obj;
     ...
     */
    private static MyHttpCookie parseInternal(String header,
                                            boolean retainHeader)
    {
        MyHttpCookie cookie = null;
    ...
    static interface CookieAttributeAssignor {
            public void assign(MyHttpCookie cookie,
                               String attrName,
                               String attrValue);
    }
    ...
    private static void assignAttribute(MyHttpCookie cookie,
                                        String attrName,
                                        String attrValue)
    {
